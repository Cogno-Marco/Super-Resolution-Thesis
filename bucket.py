...
class Bucket:
    ...
    def __init__(self, principal_voice: Photo, r: int):
        self.r: int = r
        self.k: int = len(principal_voice)
    ...
    def get_whites_count(self) -> List[List[int]]:
        whites = [[0 for _ in range(self.k)] for _ in range(self.k)]
        for photo in self.photos:
            for ind, line in enumerate(photo):
                whites[ind] = [val + count for val,
                               count in zip(line, whites[ind])]
        return [[int(round(i*self.r/len(self.photos))) for i in line] for line in whites]
