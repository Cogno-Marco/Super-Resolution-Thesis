...
repetitions: int = 100
config.CAMERA_RESOLUTION = 20
config.CAMERA_SIZE = 1600
config.WORLD_SIZE = 100_000
...
for test in range(repetitions):
    ...
    for f in offsets:
        ...
        for k in range(config.CAMERA_SIZE):
            pixelType = world.getPixelColorType(f + k*config.CAMERA_RESOLUTION, config.CAMERA_RESOLUTION)
        ...
        for macro_guessed, macro_actual in zip(guess, photoValue):
...
print(guessesProb)
guessesProb: List[float] = [guess/config.CAMERA_SIZE for guess in guessesProb]
print(guessesProb)

# upper bound formula in the thesis
upper_bound = [math.pow(math.e, -config.CAMERA_SIZE*(f**2 or 1)/(768*config.CAMERA_RESOLUTION**3)) for f in offsets]

# calculate better bound
# starting c
c = math.log(guessesProb[0], math.e) + (config.CAMERA_SIZE) / (768 * config.CAMERA_RESOLUTION**3)
for i,x in enumerate(offsets):
    c2 = math.log(guessesProb[i], math.e) + (config.CAMERA_SIZE * (x**2 or 1)) / (768 * config.CAMERA_RESOLUTION**3)
...
better_bound = [math.pow(math.e, c-config.CAMERA_SIZE*(f**2 or 1)/(768*config.CAMERA_RESOLUTION**3)) for f in offsets]
